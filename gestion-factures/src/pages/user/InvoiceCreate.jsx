...
function InvoiceCreate() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const numero = await firebaseService.getNextInvoiceNumber();
        ...
        setSnackbar({ open: true, message: `Facture ${numero} créée avec succès`, severity: 'success' });
        setTimeout(() => navigate('/factures'), 1500);
      } catch (err) {
        setSnackbar({
          open: true,
          message: err?.message || 'Erreur lors de la création de la facture',
          severity: 'error',
        });
      }
    },
  ...
  // Manual array helpers (FieldArray requires <Formik> context, not useFormik)
      ...
      >
        ...
        {clients.length === 0 && !clientsLoading && (
          <Alert severity="info" sx={{ mb: 2, borderRadius: '10px' }}>
            Aucun client. Créez d'abord des clients dans la section Clients.
          </Alert>
        )}
        <form onSubmit={formik.handleSubmit}>
          ...
          <FormControl fullWidth sx={{ ...fieldSx, mt: 2 }}>
            <InputLabel sx={{ '&.Mui-focused': { color: GOLD } }}>
              Méthode de calcul
            </InputLabel>
            <Select
              name="methode_calcul"
          ...
          {/* Remise globale (visible only méthode 3) */}
          {showRemiseGlobale && (
            <TextField
              fullWidth
              name="remise_globale"
          ...
          <Box
            sx={{
              ...
              p: 3,
              borderRadius: '12px',
              backgroundColor: 'rgba(10,15,44,0.03)',
              border: '1px solid rgba(10,15,44,0.06)',
            }}
          >
            <Typography sx={{ fontWeight: 700, color: NAVY, mb: 2, fontSize: '1rem' }}>
              Récapitulatif
            </Typography>
            ...
            {methode_calcul === 1 && (
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                ...
                <Typography sx={{ fontWeight: 700, color: GOLD, mt: 1 }}>
                  Total TTC : {formatCurrency(recapitulatif.total_ttc)}
                </Typography>
              </Box>
            )}
          ...
          <Box sx={{ mt: 4, display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
            ...
            >
              {formik.isSubmitting ? 'Création...' : 'Créer la facture'}
            </Button>
          </Box>
        </form>
      </Paper>
